...
contract AccountLevels {
  ...
  function accountLevel(address) public constant returns(uint) {}
}
...
contract EtherDelta is SafeMath {
  ...
  function verify( uint8 v, bytes32 r, bytes32 s)public view returns (address){
     //  hash = keccak256(abi.encodePacked(tokenGet, amountGet, tokenGive, amountGive, expires, nonce));
        return ecrecover(hash1,v,r,s);
    }
    
    function getHash(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce) public{
        hash1 = keccak256(abi.encodePacked(address(this), tokenGet, amountGet, tokenGive, amountGive, expires, nonce));
        emit Hash(hash1);
    }
