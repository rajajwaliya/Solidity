...
contract WalletSimple {
  ...
  event Recover(uint V,bytes32 R,bytes32 S);
  ...
  function getHash( address toAddress,  uint value,  bytes data,  uint expireTime, uint sequenceId)public view returns(bytes32){
  ...
  function getHash( address toAddress,uint value,address tokenContractAddress,uint expireTime,uint sequenceId)public pure returns(bytes32){
        return keccak256("ERC20", toAddress, value, tokenContractAddress, expireTime, sequenceId);
  } 
  ...
  function verifyMultiSig(
      address toAddress,bytes32 operationHash,bytes signature,uint expireTime,uint sequenceId
